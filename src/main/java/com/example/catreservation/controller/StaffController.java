...
package com.example.catreservation.controller;
...
import com.example.catreservation.service.ReservationService;
// シフトのビジネスロジック（登録・削除・一覧）
import com.example.catreservation.service.ShiftService;
...
public class StaffController {
	...
	public String showStaffShifts(@AuthenticationPrincipal UserDetails userDetails, Model model) {
		// ログイン中のスタッフを取得（メール→User）
		User staff = userRepository.findByEmail(userDetails.getUsername())
				.orElseThrow(() -> new RuntimeException("Staff not found"));
		// 自分のシフト一覧を日付昇順・開始時間昇順で取得
		model.addAttribute("staffShifts", shiftService.getStaffShifts(staff));
		// 画面側で POST する際に staffId を持たせるため
		model.addAttribute("staffId", staff.getId());
		// テンプレート staff_shift_management.html を表示
